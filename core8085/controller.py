...
from core8085.exceptions import OPCODENotFound
from core8085.flags import JumpFlag
from core8085.instruction_set import Instructions
from core8085.operations import Operations
from core8085.util import decompose_byte, ishex, tohex
...
class Controller:
    ...
    def _wrap_bounceable_methods(self):
        ...
        for key in self._jump_methods:
            func = self.lookup.get(key)
            if func:
                self.lookup[key] = self._skipper(func)
        return True
    ...
    def _skipper(self, func):
        ...
        def _func(*args, **kwargs):
            ...
            kwargs["bounce_to_address"] = self._bounce_to_address
            kwargs["push_return_address"] = self._push_return_address
            kwargs["pop_return_address"] = self._pop_return_address
            kwargs["resume_from_return"] = self._resume_from_return
    ...
    def _bounce_to_address(self, addr: str):
        idx, _ = self._locate_jump_address(addr)
        if idx is None:
            return False
        print(f"JUMPING to address {addr} -> {idx}")
        self._run_idx = idx
        return True

    def _push_return_address(self):
        self._return_stack.append(self._run_idx)
        return True
    ...
    def _resume_from_return(self):
        idx = self._pop_return_address()
        if idx is None:
            return False
        self._run_idx = idx
        return True
    ...
    def _locate_jump_address(self, addr: str) -> tuple:
        addr = addr.lower()
        for idx, x in enumerate(self._callstack):
            pc_val = x[3].get("pc")
            if pc_val and pc_val.lower() == addr:
                return idx, x
        return None, None
    ...
    def _parser(self, command, *args, **kwargs) -> tuple:
        ...
        _proc_command = re.split(r"[,\s]+", command.strip())
